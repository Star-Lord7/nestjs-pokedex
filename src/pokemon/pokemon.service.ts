...
export class PokemonService {
  ...
  async findOne(term: string) {
    ...
    if (!pokemon && isValidObjectId(term)) {
      // "isValidObjectId" es un metodo de Mongoose para busvcar por el MongoID
    ...
    if (!pokemon)
      throw new NotFoundException(
        `El Pokemon con el id, nombre o no ${term} no existe`,
      );
    return pokemon;
  }
  ...
  async remove(id: string) {
    ...
    if (deletedCount === 0) {
      throw new BadRequestException(
        `El Pokemon con el id ${id} no se encontro`,
      );
    }
